...
template <class I, class N, class E>
    vector<pair<I, E>> Graph<I, N, E>::getOutEdges(I const& _id) const{
        auto it = nodes.find(_id);
        if (it!=nodes.end()){
            vector<pair<I,E>> answer;
            ...
            for (auto i=it->second->children.begin(); i!=it->second->children.end(); i++){
                if(!i->second.expired())
                    answer.push_back(make_pair(i->first, i->second.lock()->value));
                else
                    erasing.push_back(i);
            }
            for (auto& i: erasing)
...
template <class I, class N, class E>
    vector<pair<I, E> > Graph<I, N, E>::getInEdges(I const& _id) const{
        auto it = nodes.find(_id);
        if (it!=nodes.end()){
            vector<pair<I,E>> answer;
            vector< typename unordered_map<I, weak_ptr<Edge> >::iterator> erasing;
...
template <class I, class N, class E>
    vector<pair<I, N> > Graph<I, N, E>::getOutNodes(I const& _id) const{
        auto it = nodes.find(_id);
        if (it!=nodes.end()){
            vector<pair<I, N> > answer;
            for (auto i=it->second->children.begin(); i!=it->second->children.end(); i++){
                if(!i->second.expired())
                    answer.push_back(make_pair(i->first, nodes.find(i->first)->second->getValue()));
            }
            return answer;
        }
        else
...
template <class I, class N, class E>
    vector<pair<I, N> > Graph<I, N, E>::getInNodes(I const& _id) const{
        auto it = nodes.find(_id);
        if (it!=nodes.end()){
            vector<pair<I, N> > answer;
            for (auto i=it->second->parents.begin(); i!=it->second->parents.end(); i++){
...
template <class I, class N, class E>
    vector<pair<I, N> > Graph<I, N, E>::getAllNodes() const{
        vector<pair<I, N> > answer;
        for (auto i=nodes.begin(); i!=nodes.end(); i++){
            answer.push_back(make_pair(i->first, i->second->getValue()));
        }
        return answer;
    }
